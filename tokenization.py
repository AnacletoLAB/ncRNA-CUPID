'''
python tokenization.py --data_dir 'foundational_GRIM/' --file_name 'test_instances.txt' --out_dir 'foundational_GRIM/tokenised_db_test_witheos'

python tokenization.py --data_dir 'foundational_GRIM/' --file_name 'train_instances.txt' --out_dir 'foundational_GRIM/tokenised_db_witheos'
'''
...
def parse_arguments():
    ...
    parser.add_argument("--tokenizer_path", type=str, default="tokenizer", help="Path to the trained AutoTokenizer.")
    ...
    parser.add_argument("--split_ratio", type=float, default=0.98, help="Train-validation split ratio.")
...
def tokenize_and_save_lines(tokenizer, input_file, train_txt_file, val_txt_file, train_bin_file, val_bin_file,is_start_with_eos, is_end_with_eos, block_size, split_ratio):
    ...
    train_lines = []
    val_lines = []
    too_long_count = 0  # Counter for lines exceeding block size
    ...
    for i, line in enumerate(train_lines_list):
        ...
        if len(ids) < block_size:
            ...
            too_long_count += 1  # Increment counter for lines over block size
            ids = ids[:block_size - 1]  # Truncate tokens that don't fit
            train_ids.extend(ids)
            train_lines.append(line.strip())
        if i % 1000000 == 0:
            print(f"now processing {i}...")
    print(f"Total lines exceeding block size ({block_size} tokens): {too_long_count}")
    too_long_count = 0
