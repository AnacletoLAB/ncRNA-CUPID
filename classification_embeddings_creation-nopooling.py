...
aug types:
python classification_embeddings_creation-nopooling.py --out_path 'db_strat/classification_aug_types_training.p' --test_data_path 'db_strat/train_instances' --ckpt_path 'model.pt' --tokenizer_path 'tokenizer'

python classification_embeddings_creation-nopooling.py --out_path 'db_strat/classification_aug_types_test.p' --test_data_path 'db_strat/test_instances' --ckpt_path 'model.pt' --tokenizer_path 'tokenizer'

"""

rna_couples_prob = 'db_strat/rna_couples_probabilities.pkl'    
special_cases_check = True
augmented_positives = True
augmented_negatives = True
use_rna_type_for_negatives = True
...
test_data_path = args.test_data_path + '-20.02.txt'
test_data_path_no_augmentation = args.test_data_path + '-20.02-noaugmentation.txt'
...
def load_rna_probs():
    ...
    with open(rna_couples_prob, 'rb') as pickle_file:
...
#true_pairs = true_pairs[:2000]
#true_pairs_no_aug = true_pairs_no_aug[:2000]
...
for element in elements:
    ...
    with torch.no_grad():
        with ctx:
            ...
            logits_target = model.extract_embeddings(x_target)
